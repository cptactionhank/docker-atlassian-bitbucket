...
describe 'Atlassian Bitbucket Server with PostgreSQL 9.3 Database' do
  ...
  include_examples 'an acceptable Bitbucket Server instance', 'using a PostgreSQL database' do
    before :all do
      ...
      if ENV['CIRCLECI']
        `docker run --link "#{@container_db.id}:db" postgres:9.3 psql --host "db" --user "postgres" --command "create database bitbucketdb owner postgres encoding 'utf8';"`
      else
        @container_db.exec ['psql', '--username', 'postgres', '--command', "create database bitbucketdb owner postgres encoding 'utf8';"]
      end
    end
    after :all do
      if ENV['CIRCLECI']
...
describe 'Atlassian Bitbucket Server with MySQL 5.6 Database' do
  include_examples 'a buildable Docker image', '.', env: ["CATALINA_OPTS=-Xms1024m -Xmx1024m -XX:+UseG1GC -Datlassian.plugins.enable.wait=#{Docker::DSL.timeout} -Datlassian.darkfeature.jira.onboarding.feature.disabled=true"]

  include_examples 'an acceptable Bitbucket Server instance', 'using a MySQL database' do
    before :all do
      ...
      # Create JIRA database
      if ENV['CIRCLECI']
        `docker run --link "#{@container_db.id}:db" mysql:5.6 mysql --host "db" --user=root --password=mysecretpassword --execute 'CREATE DATABASE bitbucketdb CHARACTER SET utf8 COLLATE utf8_bin;'`
      else
        @container_db.exec ['mysql', '--user=root', '--password=mysecretpassword', '--execute', 'CREATE DATABASE bitbucketdb CHARACTER SET utf8 COLLATE utf8_bin;']
      end
    end
    after :all do
      if ENV['CIRCLECI']
...
describe 'Atlassian Bitbucket Server behind reverse proxy' do
  ...
  include_examples 'an acceptable Bitbucket Server instance', 'using an embedded database' do
    before :all do
